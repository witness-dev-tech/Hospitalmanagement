...
export default function CreateAccount() {
  ...
  const handleRegister = async (e) => {
    e.preventDefault();
    if (formData.username.length < 4) return toast.error("Username must be 4+ characters");
    ...
    <div className="min-h-screen bg-[#F5F7FA] flex items-center justify-center p-4">
      <div className="bg-white p-8 rounded-xl shadow-md w-full max-w-md border border-gray-100">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mt-1">HIMS Registration Engine</p>
        </div>
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              <input 
                type="text" 
                className="w-full pl-10 pr-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#009688] border-gray-200 transition-all" 
          ...
          <div>
            ...
            <div className="relative">
              ...
              <select 
                className="w-full pl-10 pr-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#009688] border-gray-200 bg-white transition-all outline-none" 
                value={formData.role}
          ...
          <button 
            type="submit" 
            className="w-full py-2.5 bg-[#009688] text-white font-medium rounded-lg hover:bg-[#009688]/90 active:scale-[0.99] transition shadow-sm mt-2"
          >
            Register Account Ledger
          </button>
        </form>

        <p className="mt-6 text-center text-sm text-gray-600">
          Already registered? <Link to="/login" className="text-[#1976D2] hover:underline font-medium ml-1">Log In</Link>
        </p>
      </div>
    </div>
  );
